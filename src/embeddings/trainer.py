...
class Trainer():
    def __init__(self, model, train_dataloader, test_dataloader, training_conditions, output_dir):
        self.model = model
        self.train_dataloader = train_dataloader
        self.test_dataloader = test_dataloader
        self.training_conditions = training_conditions
        self.optimizer = torch.optim.AdamW(
            model.parameters(), lr=self.training_conditions['lr'])
        ...
        self.loss_fn = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(self.class_weights[1]).to(self.device))

        self.tester = Tester(model, test_dataloader, self.loss_fn)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.training_conditions['epochs'])):
