...
class Cleanup:
    ...
    def clean_empty_strings(self, df_dataset):
        print(
            f"Number of rows before dropping empty strings: {len(df_dataset)}")

        df_dataset = df_dataset[df_dataset['text'] != '']

        print(
            f"Number of rows after dropping empty strings: {len(df_dataset)}")
    ...
    def run(self, domain):
        ...
        df_dataset = load_dataset(
            self.dataset_raw_name, domain, split='train').to_pandas()

        tqdm.pandas()

        """

        df_dataset = self.clean_nan(df_dataset)
        df_dataset = self.clean_empty_strings(df_dataset)
        df_dataset = self.clean_duplicates(df_dataset)

        print(f"{domain} Cleaning text...")
        df_dataset['text'] = df_dataset['text'].progress_apply(self.clean_text)

        print(f"{domain} Detecting non-portuguese texts...")
        non_portuguese_indices = df_dataset['text'].progress_apply(
            self.clean_non_portuguese)
        df_dataset = df_dataset[non_portuguese_indices]

        """
        print(f"{domain} |Detecting outliers...")

        print(f"{domain} |Cleaning outliers...")
        outliers_indices = df_dataset['text'].progress_apply(
            lambda x: self.clean_outliers(x, domain))

        df_dataset = df_dataset[outliers_indices]

        hf_dataset = Dataset.from_pandas(
            df_dataset).select_columns(['text', 'label'])

        print(f"Number of rows after cleaning: {len(hf_dataset)}")

        hf_dataset.save_to_disk(
            f"{self.dataset_cleaned_name}/{domain}"
        )
