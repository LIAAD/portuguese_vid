...
def lt_corpus():
    ...
    df = pd.DataFrame({'text': brazilian_texts + portuguese_texts,
                      'label': [1] * len(brazilian_texts) + [0] * len(portuguese_texts)})
...
def kaggle_corpus():
    ...
    # Create Dataframe where brazilian texts are labeled as 1 and portuguese texts are labeled as 0
    df = pd.DataFrame(
        {'text': brazilian_texts, 'label': [1] * len(brazilian_texts)})
...
def gutemberg_project():
    ...
    # Create Dataframe where brazilian texts are labeled as 1 and portuguese texts are labeled as 0
    df = pd.DataFrame(
        {'text': portuguese_texts, 'label': [0] * len(portuguese_texts)})
...
def parse():
    ...
    all_dataset = concatenate_datasets(
        [lt_dataset, kaggle_dataset, gutember_dataset])
    ...
    print(f"Portuguese dataset stats:")
    ...
    print(f"Brazilian dataset stats:")
    print(f"Number of rows: {number_of_rows}")
    print(f"Number of tokens: {number_tokens}")
    print(f"Number of sentences: {number_sentences}")
    print(f"Number of characters: {number_characters}")
