        ...
        for pos_prob in np.arange(start_pos_prob, stop_pos_prob + 0.1, 0.1):
            for ner_prob in np.arange(0.0, 1.0 + 0.1, 0.1):
                for domain in self._DOMAINS:
                    ...
                    test_results, cross_domain_f1 = self.Tester(
                        test_dataset, best_model, train_domain=domain).test()

                    logging.info(
                        f"Cross domain f1 score: {cross_domain_f1} | test_results: {test_results}")
                    self.results.process(cross_domain_f1, domain, test_results,
